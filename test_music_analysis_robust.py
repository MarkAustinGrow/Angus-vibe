#!/usr/bin/env python3
"""
Robust test script for the enhanced music analysis functionality.

This script tests the analyze_music function with a YouTube URL and prints the results.
It includes improved error handling for JSON parsing issues.
"""
import json
import argparse
import logging
...
def main():
    ...
    try:
        ...
        # Save the analysis results to a file if requested
        if args.save:
            try:
                with open(args.save, 'w') as f:
                    json.dump(analysis_results, f, indent=2)
                logger.info(f"Analysis results saved to {args.save}")
