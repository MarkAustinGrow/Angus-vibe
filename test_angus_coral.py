#!/usr/bin/env python3
"""
Test Script for Angus Coral Integration

This script tests the integration between Angus and the Coral Protocol.
"""
import os
import sys
import logging
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('test_angus_coral.log')
    ]
)
...
def parse_args():
    ...
    Returns:
        Parsed arguments
    """
    parser = argparse.ArgumentParser(description='Test Angus Coral Integration')
    ...
    parser.add_argument(
        '--test',
        choices=['connection', 'server', 'discover', 'call', 'all'],
        default='all',
        help='Test to run'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='DID of the agent to call (for call test)'
    )
...
def main():
    ...
    try:
        ...
        if args.test == 'connection':
            ...
            test_call(coral_adapter, args.agent_did)
        elif args.test == 'server':
            run_server(coral_adapter, args.host, args.port)
        elif args.test == 'all':
            # Run all tests
            connection_success = test_connection(coral_adapter)
            if connection_success:
                discover_success = test_discover(coral_adapter)
                if discover_success:
                    call_success = test_call(coral_adapter, args.agent_did)
                    if call_success:
                        logger.info("All tests passed")
                    else:
                        logger.error("Call test failed")
                else:
                    logger.error("Discover test failed")
            else:
                logger.error("Connection test failed")
        else:
